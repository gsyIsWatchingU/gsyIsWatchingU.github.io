...
- 已将首屏视觉升级为 Three.js 非对称潮汐星云，移除人物符号与闭环旋臂，以偏心光源、单侧粒子流、暗部遮挡、景深和鼠标视差建立空间层次，并提供 Canvas 降级。
...
- 2026-09-11：首屏改为 Three.js 非对称潮汐星云，采用偏心光源、单侧粒子流、前后景粒子、冷暖光晕和暗部遮挡，并将依赖打包压缩为本地静态资源；桌面端与 390×844 手机端实景验收通过，无新增控制台错误；`npm run build`、`npm run check`、`git diff --check` 通过。
