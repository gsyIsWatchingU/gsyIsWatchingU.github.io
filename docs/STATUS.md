...
- 已将首页 Hero 重构为互动光影叙事：接入 GPU 生成的红衣少年 3D 模型，访客可移动光束触发角色警觉、逃离、躲藏与返回，留言轨迹会影响无人操作时的灯光位置。
...
- 2026-09-12：首页互动光影 Hero 已完成 GPU 网格生成、材质、骨骼与 15 段动作管线验证；最终采用稳定骨骼姿态与整体运动避免手腕穿模，桌面端、390×844 手机端、光束交互状态与控制台验收通过；`npm run build`、`npm run check`、`git diff --check` 通过。
