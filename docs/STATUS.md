...
- 已将首页 Hero 重构为互动光影叙事：接入 GPU 生成并绑定 22 根骨骼的红衣少年，访客移动光束会触发骨骼驱动的警觉、奔跑躲避、蹲伏与步行返回，留言轨迹会影响无人操作时的灯光位置。
...
- 2026-09-12：首页互动光影 Hero 已完成 GPU 网格、材质、22 骨骼与 15 段动作管线；修正动画模糊匹配与骨盆位移叠加，运行时状态已验证为 `alert → run → hide → walk → idle`，角色保持小比例并使用真实骨骼步态；桌面端与 390×844 手机端验收通过，`npm run build`、`npm run check`、`git diff --check` 通过。
