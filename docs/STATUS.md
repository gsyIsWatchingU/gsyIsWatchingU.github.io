...
- 已将首屏霓虹 SVG 星轨升级为 WebGL 粒子引力场，移除人物符号，以旋转双旋臂、粒子收束、核心呼吸和鼠标视差呈现深空尺度，并提供 Canvas 降级。
...
- 2026-09-11：首屏星海改为 WebGL 动态粒子引力场并移除中心人物；桌面端与 390×844 手机端实景验收通过，无横向溢出或控制台错误；`npm run build`、`npm run check`、`git diff --check` 通过。
