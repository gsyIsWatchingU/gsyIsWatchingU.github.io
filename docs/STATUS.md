...
- 2026-09-11：`new-prj` 接入原创雨衣男孩 Meshy GLB 模型，运行时压缩为 41,924 个三角形与 1024×1024 贴图，替代程序化方块角色；桌面端、390×844 手机端正反面、光束和长影验收通过；Meshy 免费账号骨骼生成为 PRO，本版采用代码运动且未额外消耗积分；已生成 1.5.0 离线 ZIP。
