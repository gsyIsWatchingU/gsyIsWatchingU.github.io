...
- 已将项目概览轮播的自动切换间隔缩短至 3.2 秒，并改为原位交叉淡入淡出，移除横向滑动。
...
- 2026-09-14：项目概览轮播已验证 3.2 秒自动切换与原位交叉淡入淡出；桌面端与 390×844 手机端切换无横向位移、无页面溢出，控制台无警告或错误；`npm run build`、`npm run check`、`git diff --check` 通过。
