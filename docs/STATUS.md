...
- 已补齐 CLI List 主页展示：把抽象简介改为“右键/快捷键呼出—自动携带当前目录—搜索与频率排序”的直白价值链；重新接入真实产品详情卡、2 张界面轮播和“随处唤起 / 路径不丢 / 越用越快”3 项关键能力，继续明确“仅展示 · 不开放下载”。`npm run build`、`npm run check` 通过；Chrome 实渲染验证桌面双栏、390px 移动端纵向布局、锚点偏移与轮播切换均正常，页面无横向溢出。
