...
- 已将首屏视觉收敛为 Three.js 低亮度潮汐星云，以偏心暗部、单侧粒子流和微小长衣人物建立孤独感；鼠标悬浮会牵引局部粒子并显示克制的双层引力弧，同时保留 Canvas 降级。
...
- 2026-09-11：降低 Three.js 首屏星云的曝光、饱和度和闪烁频率，将简单人物改为微小长衣剪影，并加入鼠标局部粒子偏转、暗部引力透镜和双层低亮弧；桌面端悬浮态与 390×844 手机端实景验收通过，控制台无警告或错误；`npm run build`、`npm run check`、`git diff --check` 通过。
